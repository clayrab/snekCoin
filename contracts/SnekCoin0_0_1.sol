...
library SnekCoin0_0_1 {
  ...
  function mine(LibInterface.S storage s, uint256 amount, uint256 ethAmount)
  public returns(bool){
    s.balances[msg.sender] = SafeMath.add(s.balances[msg.sender], amount);
  }
...
//s.miningPriceWei
  ...
  // doesn't complain.
  function transfer(LibInterface.S storage s, address _to, uint256 _value)
  public returns(bool) {
    ...
    require(_value <= s.balances[msg.sender]);
    s.balances[msg.sender] = SafeMath.sub(s.balances[msg.sender], _value);
  ...
  function transferFrom(LibInterface.S storage s, address _from, address _to, uint256 _value)
  public returns (bool) {
    require(_to != address(0));
    require(_value <= s.balances[_from]);
    require(_value <= s.allowed[_from][msg.sender]);
    s.balances[_from] = SafeMath.sub(s.balances[_from], _value);
    s.balances[_to] = SafeMath.add(s.balances[_to], _value);
    s.allowed[_from][msg.sender] = SafeMath.sub(s.allowed[_from][msg.sender], _value);
  ...
  function approve(LibInterface.S storage s, address _spender, uint256 _value)
  public returns (bool) {
    s.allowed[msg.sender][_spender] = _value;
